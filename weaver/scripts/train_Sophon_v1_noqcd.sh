...
testset_res34p=$(for i in $(seq -w 1075 1289); do echo -n "Res34P:${DATADIR}/Pythia/Res34P_$i.parquet "; done)
testset_qcd=$(for i in $(seq -w 0350 0419); do echo -n "QCD:${DATADIR}/Pythia/QCD_$i.parquet "; done)

ARG="--network-config networks/pheno2/example_Sophon.py -o num_classes 161 -o fc_params [(512,0.1)] \
--batch-size 512 --start-lr 5e-4 --samples-per-epoch $((10000 * 1024 / $NGPUS)) --samples-per-epoch-val $((2500 * 1024 / $NGPUS)) --num-epochs 80 --optimizer ranger \
--num-workers 5 --fetch-step 1.0 --data-split-num 200 \
--data-train $trainset_res2p $trainset_res34p \
--data-val $valset_res2p $valset_res34p \
